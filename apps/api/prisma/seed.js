...
async function main() {
  ...
  await prisma.commitmentTemplate.deleteMany();
  ...
  await prisma.commitmentTemplate.createMany({ data: commitmentTemplates });
  await prisma.commitment.createMany({ data: commitments });
  await prisma.transaction.createMany({ data: transactions });

  const [accountCount, categoryCount, goalCount, commitmentTemplateCount, commitmentCount, transactionCount] = await Promise.all([
    prisma.account.count(),
    prisma.category.count(),
    prisma.goal.count(),
    prisma.commitmentTemplate.count(),
    prisma.commitment.count(),
    prisma.transaction.count(),
  ]);

  console.log(
    `Seed complete: ${accountCount} accounts, ${categoryCount} categories, ${goalCount} goals, ${commitmentTemplateCount} commitment templates, ${commitmentCount} commitments, ${transactionCount} transactions.`,
  );
}
